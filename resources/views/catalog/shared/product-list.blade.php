...
<div class="product-card flex flex-col md:flex-row rounded-3xl bg-card">
	...
	<div class="grow flex flex-col py-8 px-6 md:px-8">
		<h3 class="text-sm lg:text-md font-black"><a href="{{route('product', ['product'=>$item->slug])}}" class="inline-block text-white hover:text-pink">{{$item->title}}</a></h3>
		<ul class="space-y-1 mt-4 text-xxs">
			@foreach ($item->formatted_properties as $property => $value)
		...
		<div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6 mt-6">
			...
			<div class="flex flex-wrap items-center gap-4">
				...
				<form 
					action="{{route(
						favorites()->isFavorite($item) ? 'favorites.delete' : 'favorites.add',
						favorites()->isFavorite($item) ? favorites()->favoriteItems()->where('product_id', $item->getKey())->first() : $item
						)}}" method="POST">
					@csrf
					@method(favorites()->isFavorite($item) ? 'DELETE' : 'POST')
					<button 
						type="submit" 
						class="w-[56px] !h-[56px] !px-0 btn btn-purple" 
						title="{{favorites()->isFavorite($item) ? 'Удалить из избранного' : 'Добавить в избранное'}}">
